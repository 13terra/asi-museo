<template>
  <div class="page" v-if="edicion">
    ...
    <section class="card links">
      ...
      <div class="link-grid">
        <router-link :to="`/gestor/ediciones/${edicion.idEdicion}/piezas`" class="btn-ghost"
          >Piezas expuestas</router-link
        >
        <router-link :to="`/gestor/ediciones/${edicion.idEdicion}/sesiones`" class="btn-ghost"
          >Sesiones</router-link
        >
        <router-link :to="`/gestor/ediciones/${edicion.idEdicion}/salas`" class="btn-ghost"
          >Salas</router-link
        >
      </div>
    </section>
...
export default {
  ...
  data() {
    return {
      edicion: null,
      exposicion: null, // ✅ FIX: Added exposicion state
      form: { nombre: "", fechaInicio: "", fechaFin: "", estado: "BORRADOR" },
      loading: true,
      saving: false,
      error: "",
      ESTADOS_EDICION
  ...
  methods: {
    ...
    async load() {
      ...
      try {
        ...
        const estado =
          this.edicion.estado || this.edicion.estadoEdicion || ESTADOS_EDICION.BORRADOR;
    ...
    async publicar() {
      ...
      try {
        ...
        // 2. ✅ FIX: Auto-activate parent Exposition if in DRAFT
        if (this.exposicion && this.exposicion.estadoExpo === "BORRADOR") {
          // Using string literal or constant
          // Update expo state
          await ExpoRepository.update(this.exposicion.idExposicion, {
            ...this.exposicion,
            estadoExpo: "ACTIVA"
          });
    ...
    async eliminar() {
      const eliminables = [
        ESTADOS_EDICION.BORRADOR,
        ESTADOS_EDICION.CANCELADA,
        ESTADOS_EDICION.FINALIZADA
      ];
...
<style scoped>
/* Styles identical to your provided file */
.page {
  max-width: 1000px;
  margin: 0 auto;
