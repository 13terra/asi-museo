  ...
  <div class="page">
    ...
    <section class="card">
      ...
      <div class="form-grid">
        ...
        <button class="btn primary" :disabled="!canAssign || saving" @click="asignar">{{ saving ? 'Guardando...' : 'Asignar' }}</button>
      </div>
    ...
    <section class="card">
      ...
      <div v-else class="table">
        <div class="row header"><span>ID usuario</span><span>Login</span><span>Permiso</span><span></span></div>
...
export default {
  ...
  data() {
    return { idExposicion: this.$route.params.idExposicion || this.$route.params.id, permisos: [], gestores: [], expoTitle: '', loading: true, saving: false, error: '', form: { idUser: '', permiso: 'EDITOR' } };
  },
  computed: { canAssign() { return this.form.idUser && this.form.permiso; } },
  ...
  methods: {
    async load() {
      ...
      } catch (e) { this.error = 'No se pudieron cargar los permisos.'; }
      finally { this.loading = false; }
    },
    async asignar() {
      if (!this.canAssign) return;
      this.saving = true; this.error = '';
      try { await GestionRepository.asignarPermiso(this.idExposicion, { ...this.form }); await this.load(); }
      catch (e) { this.error = 'No se pudo asignar el permiso.'; }
      finally { this.saving = false; }
    },
    async revocar(idUser) {
      if (!confirm('¿Revocar permiso de este gestor?')) return;
      try { await GestionRepository.revocarPermiso(this.idExposicion, idUser); await this.load(); }
      catch (e) { alert('No se pudo revocar el permiso.'); }
    }
  }
};
</script>
