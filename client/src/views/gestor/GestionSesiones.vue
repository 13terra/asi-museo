<template>
  <div class="page">
    ...
    <section class="card">
      ...
      <div class="actions">
        <button class="btn primary" :disabled="saving || !canCreate" @click="create">
      ...
      <div v-else class="grid">
        <article v-for="sesion in sesiones" :key="sesion.idSesion" class="item">
          ...
          <div v-else class="edit-grid">
            ...
            <label
              >Aforo<input type="number" min="1" max="50" v-model.number="editDraft.aforo"
            /></label>
            <div class="full sala-picker">
          ...
          <div class="item-actions">
            <template v-if="editId === sesion.idSesion">
              <button class="btn" @click="saveEdit(sesion.idSesion)" :disabled="saving">
                Guardar
              </button>
...
export default {
  ...
  methods: {
    ...
    async create() {
      ...
      this.error = "";
      try {
        // ✅ FIX: Ensure idSalas are numbers and payload is clean
        const payload = {
          ...this.form,
          idSalas: this.form.idSalas.map(Number)
        };
        await SesionRepository.create(this.idEdicion, payload);
    ...
    startEdit(sesion) {
      ...
      this.editDraft = {
        ...
        // ✅ FIX: Extract IDs correctly from object array
    ...
    async saveEdit(idSesion) {
      ...
      this.error = "";
      try {
        // ✅ FIX: Ensure idSalas are numbers for update too
        const payload = {
          ...this.editDraft,
          idSalas: this.editDraft.idSalas.map(Number)
        };
        await SesionRepository.update(idSesion, payload);
        this.cancelEdit();
        await this.load();
      } catch (e) {
