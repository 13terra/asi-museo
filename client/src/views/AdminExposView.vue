<template>
  <div class="panel-shell">
    <div class="panel-header">
      <div>
        <h2>Exposiciones (ADMIN)</h2>
        <p>Lista completa de exposiciones y creación.</p>
      </div>
    ...
    <div v-else class="cards-grid">
      ...
      <article class="expo-card" v-for="expo in expos" :key="expo.idExposicion">
        <div class="expo-top">
          <div>
            <h4>{{ expo.titulo }}</h4>
            <p class="muted">Gestores: {{ expo.gestores?.length || 0 }} · Ediciones: {{ expo.numEdiciones }}</p>
          </div>
        ...
        <div class="actions">
          ...
          <button v-if="expo.estadoExpo !== 'ARCHIVADA'" class="btn-ghost" @click="archivar(expo.idExposicion)">Archivar</button>
          <button v-else class="btn-ghost" @click="desarchivar(expo.idExposicion)">Desarchivar</button>
          <button class="btn-danger" @click="remove(expo.idExposicion)">Eliminar</button>
        </div>
      </article>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async load() {
      ...
      this.error = "";
      try {
        this.expos = await ExpoRepository.listAdmin({ incluirArchivadas: this.incluirArchivadas });
      } catch (e) {
    ...
    async archivar(id) {
      await ExpoRepository.archivar(id);
      await this.load();
    },
    async desarchivar(id) {
      await ExpoRepository.desarchivar(id);
      await this.load();
    },
    async remove(id) {
      if (!confirm("¿Eliminar la exposición?")) return;
      await ExpoRepository.remove(id);
      await this.load();
    }
  }
};
