  ...
  <div class="page">
    ...
    <section class="card">
      ...
      <div class="form-grid">
        ...
        <div class="actions">
          <button class="btn primary" :disabled="!form.nombre || saving" @click="save">
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        if (this.editId) {
          await SalaRepository.update(this.editId, { ...this.form });
        } else {
          await SalaRepository.create({ ...this.form });
        }
        this.reset();
        await this.load();
      } catch (e) {
        this.error = 'No se pudo guardar la sala.';
      } finally {
        this.saving = false;
      }
    },
