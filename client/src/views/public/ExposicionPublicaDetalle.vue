<template>
  <div class="detail-shell" v-if="!loading && expo">
    <div class="header">
      <router-link class="back" to="/">Volver al catálogo</router-link>
...
export default {
  ...
  data() {
    return { expo: null, ediciones: [], loading: true, error: '' };
  },
  async created() {
    await this.load();
  },
  methods: {
    badgeClass(estado) {
      const map = { ACTIVA: 'badge-green', EN_PREPARACION: 'badge-gray', BORRADOR: 'badge-gray', ARCHIVADA: 'badge-dark' };
      return map[estado] || 'badge-gray';
    },
    edBadge(estado) {
      const map = { PUBLICADA: 'chip chip-green', BORRADOR: 'chip chip-gray', FINALIZADA: 'chip chip-dark', CANCELADA: 'chip chip-red' };
...
<style scoped>
.detail-shell { max-width: 900px; margin: 0 auto; padding: 32px 20px 48px; display: flex; flex-direction: column; gap: 14px; }
.header { display: flex; justify-content: space-between; align-items: center; gap: 12px; }
.back { color: #1f4b99; text-decoration: none; font-weight: 700; }
.badge { align-self: flex-start; padding: 6px 10px; border-radius: 10px; font-weight: 700; font-size: 12px; }
